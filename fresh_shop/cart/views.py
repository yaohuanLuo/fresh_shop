...
def add_cart(request):
    if request.method == 'POST':
        ...
        # 组装存的数据
        good_list = [goods_id, goods_num, 1]
        ...
        if session_goods:
            ...
            request.session['goods'] = [good_list]
            count = 1
...
def del_cart_goods(request):
    if request.method == 'POST':
        goods_id = int(request.POST.get('goods_id'))
        session_goods = request.session.get('goods')
        for index in range(len(session_goods)):
            # se_good结构为[goods_id, num, is_select]
