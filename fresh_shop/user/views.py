...
def user_site(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ereors = form.errors
            return render(request, 'user_center_site.html', {'errors': ereors})


def user_info(request):
    if request.method == 'GET':
        user = request.user
        address = UserAddress.objects.filter(user_id=user.id).first()
        browse = request.session.get('browse', ())[:10]
        return render(request, 'user_center_info.html', {'address': address, 'browse': browse})
